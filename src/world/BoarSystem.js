...
export function updateBoars(level) {
  ...
  for (const old of boarsSnapshot) {
    ...
    // Fish "eats" food (shell collectibles) it swims into — emit sound event
    if (frontHitsLeaf && !e._ateFoodThisTouch) {
      e._ateFoodThisTouch = true;
      level.events?.emit("fish:ateFood", { x: e.x, y: e.y });
    } else if (!frontHitsLeaf) {
      e._ateFoodThisTouch = false;
    }
