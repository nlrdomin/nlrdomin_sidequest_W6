...
export class Game {
  ...
  _wireEventListeners() {
    ...
    // -----------------------
    if (this.sound) {
      // Collectible picked up (shell / fish food)
      ...
      this._unsubs.push(this.events.on("player:died",     () => { this.sound.play("die"); this.sound.stopMusic(); }));
      // Enemy (fish) events
      this._unsubs.push(this.events.on("boar:damaged",    () => this.sound.play("attack")));
      this._unsubs.push(this.events.on("fish:ateFood",    () => this.sound.play("fishEat")));
      // Win — pause music when done
      this._unsubs.push(this.events.on("level:won",       () => this.sound.pauseMusic()));
    }
